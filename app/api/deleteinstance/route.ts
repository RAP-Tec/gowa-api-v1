...
const AUTH_KEY = process.env.AUTH_KEY // Renomeado para consistência
const GOWA_API_KEY = process.env.GOWA_API_KEY // Carrega a chave da API GOWA do .env

// Handle POST requests to delete an instance
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.authkey || body.authkey !== AUTH_KEY) { // Usa AUTH_KEY
      return NextResponse.json(
    ...
    console.error("Error in /deleteinstance endpoint:", error)
